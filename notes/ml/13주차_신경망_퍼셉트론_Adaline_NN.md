...
- 델타 규칙 $\Delta w_j = \eta(y-\hat{y})x_j$, 오차 $(y-\hat{y}) = (-1)-(+1) = -2$, $\eta=0.2$ → 계수 $-0.4$:
- $w_0 = 0 + (-0.4)(1) = -0.4$
- $w_1 = 1 + (-0.4)(2) = 0.2$
- $w_2 = 0.5 + (-0.4)(-2) = 1.3$
- 갱신 후 $\mathbf{w} = [-0.4, 0.2, 1.3]$ — 새 판별 함수가 A와 B를 모두 올바르게 분류.

> ⚠️ **출처 주의 (★ 시험 대비)**: 강의 슬라이드의 워크드 예시는 $\Delta w = \eta\,y\,x$ 관례(미분류 시 목표값 $y$로 갱신)로 계산해 $[-0.2, 0.6, 0.9]$를 제시했다. 이는 슬라이드 텍스트가 명시한 델타 규칙 $\Delta w = \eta(y-\hat{y})x$보다 **학습률이 사실상 2배 작다**(미분류 시 $(y-\hat{y}) = 2y$). **명시된 델타 규칙을 그대로 적용하면 $[-0.4, 0.2, 1.3]$** 이다. 시험에 이 예시가 나오면 슬라이드 예시 값($[-0.2,0.6,0.9]$)을 묻는지, 공식 적용 결과($[-0.4,0.2,1.3]$)를 묻는지 확인할 것. 본 노트·퀴즈는 명시된 델타 규칙을 따른다.
