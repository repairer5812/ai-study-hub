...
// 보기 길이 균형·stem 누출 제거 교정 완료(정답 인덱스·정확성 보존). 종합세트는 label '전범위 종합'으로 🏆 표시.
...
export const FINAL_SETS = {
  ...
  [
    ...
    },
    {
      ...
      "type": "multiple_choice",
      "difficulty": "medium",
      "question": "공식 p(x)≈K/(NV)에서 두 밀도 추정기가 어느 양을 고정하느냐로 갈린다. 옳게 짝지은 것은? (N은 고정)",
      "choices": [
        "① KDE는 K를 고정해 V를 확장하고, KNN은 V를 고정해 K를 센다",
        "② KDE는 V를 고정해 K를 세고, KNN은 K를 고정해 V를 확장한다",
        "③ KDE와 KNN 모두 K와 V를 동시에 고정한 채 추정한다",
        "④ KDE와 KNN 모두 표본 수 N을 위치마다 변화시킨다"
      ],
      "answer": 1,
      "brief": "KDE: V 고정·K 카운트 / KNN: K 고정·V 확장.",
      "detailed": "KDE는 박스 사이즈(V)를 정해놓고 안에 몇 개 들었는지(K) 센다. KNN density는 K개를 채울 때까지 영역(V)을 부풀린다. 두 방법은 같은 공식 K/(NV)의 양면이며, 어느 쪽을 변수로 두느냐가 차이다. ①은 KDE와 KNN의 역할을 뒤바꿨고, ③④는 둘 다 잘못된 고정·변화 방식이다.",
    ...
    },
    {
      ...
      "choices": [
        "① 데이터가 밀집한 지역일수록 V가 커지므로 그 위치의 p(x)는 작아진다",
        "② V를 한 값으로 고정하고 K만 세므로 모든 위치에 동일한 잣대를 적용한다",
        "③ K개를 채울 때까지 V를 늘리므로 밀집 지역은 V가 작아 p(x)가 크고 희소 지역은 반대다",
        "④ K가 커질수록 추정 결과가 항상 더 noisy해지고 분산이 커져 불안정해진다"
      ],
      "answer": 2,
      "brief": "K 고정·V 확장 → 밀집=작은V·큰p, 희소=큰V·작은p (적응적).",
      "detailed": "주어진 점 x에서 정확히 K개를 포함할 때까지 영역(예: 구)을 확장한다(KNN density). 밀집 지역은 작은 V로 K개가 차서 p(x)=K/(NV)가 크고, 희소 지역은 큰 V가 필요해 p(x)가 작다. 위치마다 V를 자동 조정하므로 KDE와 달리 적응적이다. ①은 밀집/희소와 V의 관계가 반대, ②는 V 고정이라는 점에서 KDE 설명, ④는 K가 클수록 오히려 smooth해지므로 틀린다.",
    ...
    },
    {
      ...
      "detailed": "Non-parametric(KDE, KNN)은 특정 분포를 가정하지 않고 데이터 포인트 자체에 의존하며, 모델 복잡도가 데이터셋 크기에 비례해 커지고 h나 K 같은 지역 파라미터에 강하게 의존한다. Parametric(GMM)은 분포 형태(예: 정규분포)를 가정해 소수 파라미터(평균·분산 등)로 표현하며 복잡도가 데이터 크기와 무관하게 고정된다. ②는 두 접근의 성질을 뒤바꿨고, ③은 비모수의 복잡도 증가를 부정, ④는 h·K가 GMM이 아니라 비모수의 파라미터이므로 틀린다.",
    ...
    },
    {
      ...
      "choices": [
        "① J는 서로 다른 군집의 중심 사이 거리들을 합한 값으로, 클수록 좋아 최대화 대상이다",
        "② J는 총 군집 내 분산(within-cluster variance)이라 r_nk·μ_k로 최소화한다",
        "③ r_nk는 0과 1 사이를 연속으로 갖는 실수 확률값(소프트 할당)으로 정의된다",
        "④ ||x^(n) − μ_k||²는 좌표 차의 절댓값을 모두 더한 맨해튼 거리를 뜻한다"
      ],
      "answer": 1,
      "brief": "J=총 군집 내 분산, r_nk·μ_k에 대해 최소화.",
      "detailed": "노트 §2.3: J는 total within-cluster variance로, r_nk(0/1 지시변수)와 μ_k를 찾아 최소화하는 문제다. ||·||²는 제곱 유클리드 거리다. ①은 군집 간 거리·최대화라 틀리고, ③은 r_nk가 0/1 하드 할당이라 틀리며, ④는 제곱 유클리드 거리이지 맨해튼이 아니다.",
    ...
    },
    {
      ...
      "choices": [
        "① Elbow는 실루엣 점수 s_i가 가장 커지는 K를 그대로 고르는 방법이라 본다",
        "② 실루엣 점수 s_i는 정의상 항상 0 이상의 값만 가지며 음수가 될 수 없다",
        "③ 비지도학습에도 데이터와 무관하게 보편적으로 옳은 단일 K 값이 늘 존재한다",
        "④ Elbow는 W(K)(군집 내 제곱합) 대 K 곡선에서 감소가 완만해지는 팔꿈치를 K로 고른다"
      ],
      "answer": 3,
      "brief": "Elbow = W(K) 곡선의 팔꿈치(감소가 완만해지는 지점).",
      "detailed": "노트 §2.6: Elbow는 W(K)=Σ_k Σ_{x∈C_k}||x−μ_k|| 대 K 그래프의 팔꿈치를 고른다. 비지도라 보편적으로 옳은 K는 없고, s_i는 [−1,1] 범위로 음수도 가능하다. ①은 Elbow를 실루엣 정의로 바꿔 틀리고, ②는 s_i가 음수도 되므로, ③은 보편적 정답 K가 없으므로 틀린다.",
    ...
    },
    {
      ...
      "type": "multiple_choice",
      "difficulty": "medium",
      "question": "공분산 행렬 S에 대해 S u_1 = λ_1 u_1 일 때 λ_1과 u_1의 의미로 옳은 것은?",
      "choices": [
        "① u_1이 그 방향의 분산값을 담고, λ_1이 주성분 방향을 가리키는 벡터에 해당한다",
        "② λ_1은 항상 0이 되어 아무 의미가 없고 오직 u_1만 정보를 담는다",
        "③ λ_1은 데이터의 클래스 라벨에 해당하고, u_1은 데이터의 평균 벡터에 해당한다",
        "④ u_1은 주성분 방향인 고유벡터, λ_1은 그 방향 분산값이다 (u_1^T S u_1 = λ_1)"
      ],
      "answer": 3,
      "brief": "u_1=주성분 방향(고유벡터), λ_1=그 방향 분산값(고유값). u_1^T S u_1=λ_1.",
      "detailed": "노트 § 2.5: u_1은 S의 고유벡터로 주성분 방향, λ_1은 고유값으로 그 방향의 분산 값 그 자체다. u_1^T를 왼쪽 곱하고 u_1^T u_1=1을 쓰면 u_1^T S u_1=λ_1. 분산 최대화는 가장 큰 고유값을 갖는 고유벡터를 제1주성분으로 택하는 것이다. ①은 λ와 u의 역할을 뒤바꿨고, ②는 λ_1이 0이라는 점에서, ③은 라벨·평균이라는 점에서 틀린다.",
    ...
    },
    {
      ...
      "type": "multiple_choice",
      "difficulty": "medium",
      "question": "ICA가 x = A s를 복원해도 남는 내재적 모호성(ambiguity)과, PCA와의 대비로 옳은 것은?",
      "choices": [
    ...
    },
    {
      ...
      "choices": [
        "① z_i와 b_i 모두 데이터 포인트마다 서로 다른 값을 갖는 데이터별 계수다",
        "② z_i는 살려두는 M개 차원으로 데이터마다 다르고, b_i는 버려지는 D−M개 차원의 공통 상수다",
        "③ z_i는 버려지는 차원의 모든 데이터 공통 상수이고, b_i는 살려두는 차원의 데이터별 값이다",
        "④ z_i와 b_i 모두 데이터와 무관하게 모든 점에 공통으로 적용되는 고정 상수다"
      ],
      "answer": 1,
      "brief": "z_i=살림(데이터마다 다름), b_i=버림(모든 데이터 공통 상수).",
      "detailed": "노트 § 3.3: z_i는 살려두는 M개 차원으로 데이터마다 값이 다름(개별 특성 반영), b_i는 버려지는 D−M개 차원으로 모든 데이터 공통의 고정 상수다. Step 1에서 z_i=(x)^T u_i, b_i=x̄^T u_i. ①은 b_i까지 데이터별이라 본 점, ③은 z_i와 b_i의 역할을 뒤바꾼 점, ④는 z_i까지 상수로 본 점에서 틀린다.",
    ...
    },
    {
      ...
      "choices": [
        "① 저차원 t-분포의 두꺼운 꼬리가 중간~먼 거리 점에 더 큰 q_ij를 줘서 적당히 퍼진 채로 둔다",
        "② 저차원 t-분포가 가우시안보다 모든 점을 한 점에 더 강하게 끌어모아 군집을 압축한다",
        "③ 저차원 가우시안과 t-분포가 사실상 같은 모양이라 어느 분포를 써도 차이가 없다",
        "④ 저차원에서도 P와 같은 가우시안을 그대로 써야 crowding 문제를 막을 수 있게 된다"
      ],
      "answer": 0,
    ...
    },
    {
      ...
      "choices": [
        "① 가장 가까운 이웃까지의 거리 ρ_i와 같게 σ_i = ρ_i 로 고정해 둔다",
        "② t-SNE에서처럼 perplexity = 2^{H} 를 만족하도록 σ_i 값을 정한다",
        "③ 자동으로 정하는 식이 없어 σ_i를 사용자가 매번 직접 입력해야 한다",
        "④ Σ_{j∈N_k(i)} exp(−(d − ρ_i)/σ_i) ≈ log_2(k) 를 만족하도록 σ_i를 정한다"
      ],
      "answer": 3,
      "brief": "k 이웃 가중치 합 ≈ log_2(k) 를 만족하게 σ_i 자동 결정.",
      "detailed": "노트 §5.2: k를 정하면 Σ_{j∈N_k(i)} exp(−(d−ρ_i)/σ_i) ≈ log_2(k)를 만족하도록 σ_i가 자동 결정된다. ①은 σ_i를 ρ_i로 고정한다는 점에서, ②의 perplexity(2^H)는 t-SNE의 개념이라 혼동이며, ③은 자동 결정 식이 존재하므로 틀린다.",
    ...
    },
    {
      ...
      "choices": [
        ...
        "③ Axon(축삭돌기)은 외부 입력 신호의 수신을 맡으므로 입력 x에 대응한다",
        "④ Dendrite·Cell Body·Axon 세 구조가 모두 가중치 w 하나에만 대응한다"
      ],
      "answer": 0,
      "brief": "Cell Body=신호 통합(합산)→가중합.",
      "detailed": "슬라이드: signals arrive at the dendrites(입력 수신), are integrated in the cell body(합산), and if the combined signal exceeds a threshold, an output is generated along the axon(임계값 넘으면 출력). 즉 Dendrite=입력 x, Cell Body=가중합 Σ, Axon=Step function 출력. ②는 Dendrite를 출력으로, ③은 Axon을 입력으로 뒤바꿨고, ④는 세 구조를 가중치 하나로 뭉뚱그려 틀린다.",
    ...
    },
    {
      ...
      "choices": [
        ...
        "④ 퍼셉트론은 임계값(step) 통과 후 클래스 예측으로, Adaline은 통과 전 연속 선형 출력 σ(z)=z로 에러를 계산한다"
      ],
      "answer": 3,
      "brief": "퍼셉트론=step 통과 후 클래스, Adaline=통과 전 연속값 σ(z)=z.",
      "detailed": "슬라이드: Adaline does not use the thresholded class prediction directly to update the weights. Instead it uses a linear activation σ(z)=z(identity). 핵심: 어느 시점에서 에러를 계산하는가. Adaline은 연속값이라 미분 가능 → 경사하강 사용. 임계값 함수는 학습 후 최종 분류 때는 여전히 사용(학습과 분류 분리). ①은 퍼셉트론과 Adaline의 시점을 뒤바꿨고, ②는 둘 다 통과 후라 본 점, ③은 분류 단계에서도 안 쓴다고 본 점에서 틀린다.",
    ...
    },
    {
      ...
      "detailed": "노트: 각 샘플이 베르누이 분포를 따른다 가정, 우도 p(Y|X)=∏ ŷ^y (1-ŷ)^(1-y). 라벨이 독립적으로 추출되어 곱(factorization) 가능. MLE로 우도 최대화 → 계산 편의 위해 음의 로그우도(NLL) 취함 → 이 NLL이 BCE와 완전히 동일. 즉 이진 분류에서 NLL = BCE. ②는 가우시안이라 본 점, ③은 우도를 줄인다고 본 점, ④는 라벨 독립이 전제인데 종속이라 한 점에서 틀린다.",
  ...
  [
    ...
    },
    {
      ...
      "choices": [
        "① K가 작으면(K=1) over-smoothed되고 K가 크면(K=30) noisy해져 각 점 위로 뾰족해진다",
        "② K는 평활도에 전혀 영향을 주지 않아 추정 결과가 K 값과 무관하게 늘 동일하게 나온다",
        "③ K가 작으면(K=1) noisy해 각 점 위로 뾰족하고 K가 크면(K=30) over-smoothed되어 디테일이 사라진다",
        "④ K가 클수록 분산이 줄어 추정이 항상 더 정확하므로 K는 최대로 두는 편이 좋다"
      ],
      "answer": 2,
    ...
    },
    {
      ...
      "choices": [
        ...
        "④ 밀집·희소 지역에 같은 h를 써 한쪽은 oversmooth·다른 쪽은 noisy해진다"
      ],
      "answer": 3,
    ...
    },
    {
      ...
      "choices": [
        "① 파라미터는 π_k, μ_k, Σ_k 세 개이며 0 ≤ π_k ≤ 1 이고 Σ_k π_k = 1 을 만족한다",
        "② 파라미터는 π_k, μ_k, Σ_k 세 개이나 π_k는 음수도 허용되는 임의의 실수면 된다",
        "③ 파라미터는 μ_k, Σ_k 두 개뿐이며 가중치 π_k는 모든 성분에서 항상 1로 고정된다",
        "④ 파라미터는 π_k, μ_k 두 개이고 Σ_k는 모든 성분에서 항상 단위행렬로 고정된다"
      ],
      "answer": 0,
    ...
    },
    {
      ...
      "choices": [
        "① 할당 단계에서 중심 μ_k를 갱신하고, 갱신 단계에서 할당 r_nk를 갱신해 순서가 반대다",
        "② 두 변수(할당 r_nk와 중심 μ_k)를 고정 없이 동시에 한 번에 닫힌 형태로 최적화한다",
        "③ 할당 단계는 μ_k를 고정해 가장 가까운 중심으로 r_nk 갱신, 갱신 단계는 r_nk 고정해 μ_k를 평균으로 갱신",
    ...
    },
    {
      ...
      "choices": [
        ...
        "③ 군집 수 K를 학습 후에만 정할 수 있다는 점 + 계층적 트리를 반드시 만들어야 한다는 점",
    ...
    },
    {
      ...
      "choices": [
        ...
        "③ a_i와 b_i 모두 전체 데이터셋의 모든 점까지의 평균 거리로 정의되어 동일하게 계산된다",
    ...
    },
    {
      ...
      "choices": [
        "① μ_k는 책임값 γ를 전혀 쓰지 않은 모든 점의 단순 산술평균으로 갱신된다",
        "② μ_k = (1/N_k) Σ_n γ(z_nk) x^(n) 이고 혼합계수 π_k = N_k / N 으로 갱신된다",
        "③ 혼합계수는 π_k = N / N_k 이며 책임값 합이 클수록 작은 가중을 받는다",
        "④ Σ_k는 책임값 γ 없이 (x−μ_k)(x−μ_k)^T 를 모든 점에 대해 단순 합산해 구한다"
      ],
      "answer": 1,
    ...
    },
    {
      ...
      "choices": [
        "① x̂ = Σ_{i=1}^{M} z_i u_i 로, 투영 성분만 더하고 평균 x̄는 더하지 않는다",
        "② x̂ = x̄ − Σ_{i=1}^{M} z_i u_i 로, 평균에서 투영 성분을 빼서 복원한다",
        "③ x̂ = Σ_{i=1}^{D} λ_i u_i 로, 고유값 λ_i를 가중치로 전체 축을 더한다",
        "④ x̂ = x̄ + Σ_{i=1}^{M} z_i u_i 로, 평균 x̄를 반드시 더해 원래 위치로 복원한다"
      ],
      "answer": 3,
    ...
    },
    {
      ...
      "choices": [
        "① S u_1 = λ_1 u_1 로, u_1이 공분산 S의 고유벡터가 되는 고유값 문제",
        "② S = λ_1 I 로, 공분산 행렬 자체가 단위행렬의 스칼라 배가 되는 조건",
        "③ u_1 = λ_1 S 로, 방향 벡터가 공분산 행렬의 스칼라 배가 되는 조건",
    ...
    },
    {
      ...
      "choices": [
        "① unmixing 행렬 W가 항상 직교 행렬이 되도록 제약을 거는 정규화 항이기 때문",
        "② 소스 신호 s가 반드시 가우시안 분포를 따르도록 강제하기 위한 보정 항이기 때문",
        "③ s = W x 변환이 공간 부피를 바꾸므로, 확률 총합 1을 유지하기 위한 자코비안 부피 보정 항이기 때문",
        "④ 경사 상승의 학습률 α를 우도 규모에 맞춰 정규화하기 위한 스케일 항이기 때문"
      ],
      "answer": 2,
    ...
    },
    {
      ...
      "choices": [
        "① p_ij가 0에 가까운 멀리 떨어진 쌍이 페널티의 대부분을 지배하게 된다",
        "② KL은 대칭이라 P와 Q를 서로 바꿔 KL(Q||P)로 써도 값이 동일하게 유지된다",
        "③ p_ij가 큰(가까운) 쌍에서 q_ij를 작게 두면 큰 페널티가 부과되어 가까운 이웃을 저차원에서도 가깝게 유지한다",
        "④ q_ij가 커질수록 log 비가 항상 증가해 손실이 단조적으로 커지게 된다"
      ],
      "answer": 2,
    ...
    },
    {
      ...
      "choices": [
        "① 연결이 강한(w≈1) 쌍을 저차원에서 더 강하게 끌어당겨 뭉치게 하는 인력 역할을 한다",
        "② 모든 쌍에서 항상 0으로 떨어져 손실에 기여하지 않고 무시되는 더미 항이다",
        "③ 첫 항과 합치면 KL Divergence와 정확히 동일한 의미·값을 갖는 항이다",
        "④ 연결이 약한(w≈0) 쌍이 저차원에서 가까워지면(v 큼) 페널티를 부과해 밀어내는(repulsion) 역할을 한다"
      ],
      "answer": 3,
    ...
    },
    {
      ...
      "choices": [
        "① 지수화(e^z로 모든 값을 양수화)와 정규화(전체 합으로 나눠 합을 1로 만들어 확률 분포화)",
  ...
  [
    ...
    },
    {
      ...
      "choices": [
        ...
        "② Box 커널은 경계에서 1↔0으로 끊기고, Gaussian 커널은 거리에 따라 부드럽게 감소하며 h가 σ 역할을 한다",
    ...
    },
    {
      ...
      "choices": [
        "① z는 관측되지 않는 잠재변수로 z_k=1이면 k컴포넌트가 선택되고, marginalize하면 GMM p(x)가 복원된다",
        "② z는 직접 관측되는 변수이며 그 선택 확률이 p(z_k=1)=μ_k 로 평균 벡터에 의해 정의된다",
        "③ z는 one-hot이 아닌 연속 실수 벡터이며 그 기댓값(평균)이 혼합계수 π_k와 같다",
        "④ z는 잠재변수가 아니라 입력 x 그 자체여서 데이터로부터 직접 관측이 가능하다"
      ],
      "answer": 0,
      "brief": "z=one-hot 잠재변수, z_k=1이면 k컴포넌트, marginalize → GMM 복원.",
    ...
    },
    {
      ...
      "choices": [
        ...
        "③ GMM은 군집 수 K를 사용자가 주지 않아도 학습 중 사후에 자동으로 결정한다",
        "④ GMM은 군집마다 고유 공분산 Σ_k를 가져 모양·크기·방향이 자유롭고 소프트 할당을 하는 완전 확률 모델이다"
      ],
      "answer": 3,
    ...
    },
    {
      ...
      "choices": [
        "① 뒤쪽 차원일수록 고유값(분산)이 급감해 정보가 거의 없으므로, 스펙트럼이 꺾이는 지점으로 M을 정한다",
        "② 고유값은 인덱스가 커질수록 함께 커지므로 가장 뒤쪽 차원들이 분산을 가장 많이 설명한다",
        "③ 모든 차원의 고유값이 정확히 동일해 스펙트럼이 평평하므로 M 선택에 쓸모가 없다",
        "④ 고유값이 음수로 바뀌는 지점이 반드시 존재하므로 그 인덱스에서 M을 정한다"
      ],
      "answer": 0,
    ...
    },
    {
      ...
      "choices": [
        ...
        "② 공분산 행렬 S가 대칭이며, 대칭 행렬에서 서로 다른 고유값의 고유벡터는 자연히 직교하기 때문",
    ...
    },
    {
      ...
      "choices": [
        "① PCA: N×N 그램(샘플 간), MDS: p×p 공분산(변수 간)",
        "② PCA·MDS 모두 변수 간 관계인 p×p 행렬을 분해한다",
        "③ PCA·MDS 모두 샘플 간 관계인 N×N 행렬을 분해한다",
        "④ PCA: p×p 공분산(변수 간), MDS: N×N 그램(샘플 간)"
      ],
      "answer": 3,
    ...
    },
    {
      ...
      "choices": [
        ...
        "② v = X u 로, PCA 고유벡터를 데이터 X로 사상한 것이다",
        "③ v = X^T u 로, X의 전치를 곱해 변수 공간으로 보낸 것이다",
        "④ v = u / λ 로, 고유값 크기로 나눠 정규화한 것이다"
      ],
      "answer": 1,
    ...
    },
    {
      ...
      "choices": [
        ...
        "② 대칭화가 t-SNE는 fuzzy-set union, UMAP은 (p+p)/2N 평균으로 서로 반대다",
    ...
    },
    {
      ...
      "choices": [
        "① wᵀx=1.5 > 0 → ŷ=+1, 정답 y와 불일치 → 가중치 갱신 필요",
        "② wᵀx=1.5 > 0 → ŷ=+1, 정답 y와 일치 → 갱신 불필요(Δw=0)",
        "③ wᵀx=−1.5 < 0 → ŷ=−1, 정답과 달라 오분류 → 갱신 필요",
        "④ wᵀx=0 이라 결정 경계 위에 놓여 결과와 무관하게 무조건 갱신"
      ],
      "answer": 1,
  ...
  [
    {
      ...
      "choices": [
        "① N이 크면 var[K/N]=P(1−P)/N → 0 이라 K/N이 P로 집중하기 때문",
        "② K가 항상 N과 같아 비율 K/N이 1로 고정되기 때문",
        "③ R을 매우 작게 잡으면 그 안에서 p(x)가 상수가 되기 때문",
        "④ 이항분포가 큰 N에서 가우시안 분포 모양으로 수렴하기 때문"
      ],
      "answer": 0,
    ...
    },
    {
      ...
      "choices": [
        "① h가 작으면 over-smoothing되고, 크면 점마다 spiky해진다",
        "② h가 작으면 점마다 뾰족한 봉우리로 noisy하고, 크면 over-smoothing된다",
        "③ h는 추정 곡선의 부드러움 정도에 아무런 영향을 주지 않는다",
        "④ h는 클수록 항상 최적의 density model을 만들어 준다"
      ],
      "answer": 1,
    ...
    },
    {
      ...
      "choices": [
        "① N이 매우 클 때 P가 비율 K/N으로 수렴한다는 의미",
        "② 영역 R이 매우 클 때 그 안의 p(x)가 균일해진다는 의미",
        "③ R이 매우 작아 p(x)를 상수로 보아 ∫_R p(x)dx ≈ p(x)·V 가 되기 때문",
        "④ K가 고정된 채 영역 부피 V만 무한히 커진다는 의미"
      ],
      "answer": 2,
    ...
    },
    {
      ...
      "choices": [
        ...
        "② 모든 클래스의 표본 수 N_c가 서로 같다고 가정했기 때문",
        "③ 이웃 수를 K=1로 고정해 단일 최근접점만 보기 때문",
        "④ Bayes에 세 밀도식을 대입하면 N_c·N·V 항이 약분돼 K_c/K만 남기 때문"
      ],
      "answer": 3,
    ...
    },
    {
      ...
      "choices": [
        "① 로그 안의 합으로 closed-form 해가 없음 / 특이점에서 likelihood가 ∞로 발산",
    ...
    },
    {
      ...
      "choices": [
        "① 비용 J가 매 단계 단조 감소해 항상 전역 최적에 수렴한다",
        "② 비용 J가 단계마다 진동할 수 있어 수렴이 보장되지 않는다",
        "③ J는 단조 감소해 수렴하나 초기값에 따라 지역 최적에 빠질 수 있다",
        "④ 비용 J가 매 단계 증가하다가 어느 순간 멈춰 정지한다"
      ],
      "answer": 2,
    ...
    },
    {
      ...
      "choices": [
        "① 점 x가 컴포넌트 k에서 나왔을 사후확률이며 분모가 정규화해 Σ_k γ=1이다",
        "② γ(z_k)는 0 또는 1 값만 갖는 하드(hard) 지시 변수다",
        "③ γ(z_k)는 분모 없이 π_k N(x|μ_k,Σ_k)만으로 정의된 값이다",
        "④ γ(z_k)는 M-step에서 파라미터를 고정하지 않은 채로 계산한다"
      ],
      "answer": 0,
    ...
    },
    {
      ...
      "choices": [
        "① 공분산 S → 고유분해 → 오름차순 정렬 → 가장 작은 M개 선택",
        "② 공분산 S → 고유분해 → 내림차순 정렬 → 가장 큰 M개로 투영",
        "③ 거리 행렬 → 더블 센터링 → 가장 작은 M개 좌표 선택",
    ...
    },
    {
      ...
      "choices": [
        ...
        "③ 네 기법 모두 거리 행렬 D를 입력으로 받아 임베딩한다",
    ...
    },
    {
      ...
      "choices": [
        "① 각 점의 분산 폭 σ_i를 직접 결정하기 위해 N으로 나눈다",
        "② KL Divergence 값을 음수가 되도록 만들기 위해 나눈다",
        "③ 조건부 p_(j|i)들의 결합확률 전체 합이 1이 되게 정규화하려고",
        "④ t-분포의 자유도를 정확히 1로 고정하기 위해 나눈다"
      ],
      "answer": 2,
    ...
    },
    {
      ...
      "choices": [
        "① 모든 이웃 가중치 w_ij를 일률적으로 0으로 만들어 버린다",
        "② 거리를 음수로 바꿔 점 사이에 척력(repulsion)을 만든다",
        "③ t-분포 꼬리의 자유도를 결정해 임베딩 분포를 정한다",
        "④ 최근접 이웃에서 분자가 0이 돼 w≈1, 밀도 무관 최소 한 이웃과 강연결"
      ],
      "answer": 3,
    ...
    },
    {
      ...
      "choices": [
        "① step function의 미분이 모든 점에서 항상 1로 일정하기 때문",
        "② 선형 활성 σ(z)=z의 연속 출력이라 미분 가능해 MSE 경사하강이 되기 때문",
    ...
    },
    {
      ...
      "choices": [
        "① ŷ=0.9(정답에 가까울 때)일 때 BCE 페널티가 가장 크다",
        "② ŷ=0.1일 때 BCE와 MSE 페널티가 동일하게 0.81로 같다",
        "③ ŷ가 작아져 정답에서 멀어질수록 BCE 페널티는 점점 줄어든다",
        "④ ŷ=0.1로 강하게 틀리면 BCE≈2.302로 급증해 MSE 0.81보다 크다"
      ],
      "answer": 3,
    ...
    },
    {
      ...
      "choices": [
        "① ∂L/∂w_21=(ŷ1−y1)x2 처럼 '예측−정답'형이라 오차가 클수록 더 갱신한다",
        "② ∂L/∂w가 입력 x만으로 정리돼 예측·정답 값과는 전혀 무관하다",
        "③ ∂L/∂w가 정답 y의 제곱 항으로 정리되어 입력과 무관해진다",
  ...
  [
    {
      ...
      "choices": [
        "① KDE는 영역 V를 고정하고 그 안의 K를 세며, KNN은 K를 고정하고 V를 확장한다",
        "② KDE는 K를 고정하고 V를 확장하며, KNN은 V를 고정하고 K를 센다",
        "③ KDE는 영역 V를 확장하며, KNN은 표본 수 N을 고정한다",
        "④ KDE와 KNN 모두 K와 V를 함께 고정해 밀도를 구한다"
      ],
      "answer": 0,
      "brief": "KDE=V 고정·K 카운트, KNN=K 고정·V 확장.",
    ...
    },
    {
      ...
      "choices": [
        "① r_nk와 μ_k를 한 번에 닫힌 해로 동시에 구해 종료한다",
        "② μ_k 고정해 r_nk 갱신, r_nk 고정해 μ_k 갱신을 번갈아 반복한다",
        "③ 라그랑주 승수로 혼합계수 π_k를 반복 갱신해 수렴시킨다",
        "④ 덴드로그램을 만든 뒤 잘라서 군집 수 K를 결정한다"
      ],
      "answer": 1,
    ...
    },
    {
      ...
      "choices": [
        "① u₁ = S⁻¹1, 즉 공분산의 역행렬로 주성분이 결정된다",
        "② S u₁ = λ₁u₁, 즉 u₁은 공분산 S의 고유벡터이고 λ₁은 그 분산값이다",
        "③ S u₁ = 0, 즉 영벡터 해만 존재해 주성분이 없다",
        "④ u₁ᵀu₁ = λ₁, 즉 제약식 자체가 고유값과 같아진다"
      ],
      "answer": 1,
    ...
    },
    {
      ...
      "choices": [
        "① PCA는 그램 XXᵀ를, MDS는 공분산 XᵀX를 분해하며 둘은 무관하다",
        "② PCA는 비선형, Classical MDS는 선형이라 결과가 항상 다르다",
        "③ PCA는 공분산 XᵀX를, MDS는 그램 XXᵀ를 분해하나 유클리드면 동일해진다",
    ...
    },
    {
      ...
      "choices": [
        "① t-SNE와 UMAP 둘 다 단순 평균 (p+p)/2N 방식으로 대칭화한다",
        "② t-SNE는 fuzzy-set union으로, UMAP은 단순 평균으로 대칭화한다",
        "③ t-SNE와 UMAP 둘 다 max(·,·) 연산만으로 대칭화한다",
        "④ t-SNE는 단순 평균 (p+p)/2N, UMAP은 fuzzy-set union으로 대칭화한다"
      ],
      "answer": 3,
    ...
    },
    {
      ...
      "choices": [
        "① ŷ1=1이 되고, softmax의 역할은 미분과 적분이다",
        "② ŷ1=0이 되고, softmax의 역할은 직교화와 센터링이다",
        "③ ŷ1=2가 되고, softmax의 역할은 분산 최대화와 거리 보존이다",
        "④ ŷ1=0.5가 되고, softmax의 역할은 지수화(양수화)와 정규화(합=1)이다"
      ],
      "answer": 3,
    ...
    },
    {
      ...
      "choices": [
        "① K_c/K, 즉 K 이웃 중 클래스 비율로 다수결 분류가 밀도추정+Bayes의 귀결이다",
        "② N_c/N, 즉 클래스 prior를 그대로 사후확률로 사용한다",
        "③ K/(NV), 즉 무조건부 밀도를 그대로 사후확률로 사용한다",
        "④ π_k N(x|μ_k,Σ_k), 즉 컴포넌트 밀도를 그대로 사후확률로 쓴다"
      ],
      "answer": 0,
    ...
    },
    {
      ...
      "choices": [
        ...
        "② 로그 안 합으로 닫힌 해가 없는 문제와 σ_k→0에서 발산하는 특이점 문제",
    ...
    },
    {
      ...
      "choices": [
        "① 책임값 γ(z_k)와 지시변수 r_nk 둘 다 0 또는 1의 하드 값이다",
        "② r_nk가 연속 확률값이고 γ(z_k)가 그 하드(0/1) 버전이다",
        "③ γ(z_k)는 사후확률로 [0,1]·합=1인 소프트 값이며 r_nk(0/1)의 소프트 버전이다",
        "④ 책임값 γ(z_k)와 지시변수 r_nk는 서로 무관한 별개의 양이다"
      ],
      "answer": 2,
    ...
    },
    {
      ...
      "choices": [
        "① λ=N 이고 갱신식은 π_k = N/N_k 이다",
        "② λ=0 이고 갱신식은 π_k = 1/K (균등)이다",
        "③ λ=−1 이고 갱신식은 π_k = γ(z_nk) 이다",
        "④ λ=−N 이고 갱신식은 π_k = N_k/N (N_k=Σ_n γ)이다"
      ],
      "answer": 3,
      "brief": "λ=−N, π_k=N_k/N.",
    ...
    },
    {
      ...
      "choices": [
        "① 둘 다 ‖μ_A−μ_B‖² 기반이나 Ward는 크기가중 |A||B|/(|A|+|B|)가 붙는다",
    ...
    },
    {
      ...
      "choices": [
        "① x̂ = Σ z_i u_i, 즉 평균을 더하지 않고 그대로 복원한다",
        "② x̂ = x̄ + Σ z_i u_i, 즉 평균 x̄를 더해야 원위치로 복원된다",
        "③ x̂ = x̄ − Σ z_i u_i, 즉 평균에서 성분을 빼서 복원한다",
        "④ x̂ = Σ_{i>M} b_i u_i, 즉 버린 차원으로 복원한다"
      ],
      "answer": 1,
    ...
    },
    {
      ...
      "choices": [
        ...
        "④ 두 정의 모두 가장 작은 M개 고유값의 고유벡터를 보존한다"
      ],
      "answer": 2,
    ...
    },
    {
      ...
      "choices": [
        "① D를 변환 없이 그대로 고유분해해 좌표를 X=D로 둔다",
        "② D를 공분산 XᵀX로 바꾼 뒤 소프트맥스를 거쳐 X를 복원한다",
        "③ D에 경사하강법을 적용해 B를 학습한 뒤 X=B⁻¹로 둔다",
        "④ D를 더블센터링→그램 B=XXᵀ→B=VΛVᵀ 분해→X=VΛ^{1/2}로 복원한다"
      ],
      "answer": 3,
    ...
    },
    {
      ...
      "choices": [
        "① h나 K가 작으면 noisy, 크면 over-smoothing되어 디테일이 사라진다",
        "② h가 작을수록, K가 클수록 추정 결과가 noisy(spiky)해진다",
        "③ h와 K는 추정 결과의 매끄러움에 전혀 영향을 주지 않는다",
        "④ h가 크면 noisy해지고, K가 작으면 over-smoothed가 된다"
      ],
      "answer": 0,
    ...
    },
    {
      ...
      "choices": [
        ...
        "② K-Means·GMM·t-SNE·UMAP은 K를 미리 안 정하고 덴드로그램을 잘라 정한다",
