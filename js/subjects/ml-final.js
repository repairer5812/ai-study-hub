...
export const FINAL_SETS = {
  ...
  [
    {
      ...
      "choices": [
        ...
        "② 레이블 없이 데이터에 내재된 패턴·규칙성·숨은 구조를 스스로 발견한다",
    ...
    },
    {
      ...
      "choices": [
        ...
        "④ $u_1$은 주성분 방향인 고유벡터이고, $\\lambda_1$은 그 방향의 분산값이다"
      ],
      "answer": 3,
  ...
  [
    ...
    },
    {
      ...
      "choices": [
        ...
        "④ 군집이 구형·동일 크기라는 경직된 가정을 두고, 비확률적 하드 할당이라 겹침에 약하다"
      ],
      "answer": 3,
    ...
    },
    {
      ...
      "choices": [
        ...
        "③ SGD는 미니배치의 한 종류로, 배치 크기를 전체 데이터의 절반으로 고정한 방식이다",
  ...
  [
    ...
    },
    {
      ...
      "choices": [
        ...
        "② Box 커널은 경계에서 1↔0으로 끊기고, Gaussian 커널은 거리에 따라 부드럽게 감소한다",
    ...
    },
    {
      ...
      "choices": [
        ...
        "④ GMM은 군집마다 고유 공분산 $\\Sigma_k$를 가져 타원형 군집을 다루고 소프트 할당을 한다"
      ],
      "answer": 3,
    ...
    },
    {
      ...
      "choices": [
        "① 분산을 최대화하는 정의와 분산을 최소화하는 정의 두 가지로, 둘은 같은 고유벡터로 귀결된다",
    ...
    },
    {
      ...
      "choices": [
        ...
        "② 분리가 극명할수록 저차원 임베딩이 원본의 거리·밀도 구조를 그만큼 정확히 보존한 것이다",
        "③ UMAP의 극명한 분리는 학습 과정에서 클래스 라벨을 직접 사용했기 때문에 나타난 결과다",
        "④ 극명한 분리는 곧 그 군집들이 실제 데이터 공간에서도 그만큼 멀리 떨어져 있음을 보장한다"
      ],
      "answer": 0,
    ...
    },
    {
      ...
      "choices": [
        "① MSE는 출력 확률을 0 또는 1로 반올림해 미세한 오차 정보를 잃기 때문이다",
        "② BCE는 연속값 회귀에, MSE는 범주형 분류에 각각 더 적합하기 때문이다",
        "③ BCE는 출력층 활성함수가 필요 없어 계산이 더 단순하기 때문이다",
        "④ MSE는 오분류에도 그래디언트가 약하지만, BCE는 확신에 찬 오분류에 훨씬 큰 페널티를 준다"
      ],
      "answer": 3,
  ...
  [
    ...
    },
    {
      ...
      "answer": 2,
      "brief": "델타 규칙 $\\Delta w=\\eta(y-\\hat{y})x = -0.4x$ → $[-0.4, 0.2, 1.3]$. (슬라이드 예시 $[-0.2,0.6,0.9]$는 $\\eta\\,y\\,x$ 관례)",
      "detailed": "오차 $(y-\\hat{y}) = (-1)-(+1) = -2$, 학습률 $\\eta=0.2$이므로 $\\Delta w_j = \\eta(y-\\hat{y})x_j = 0.2\\cdot(-2)\\cdot x_j = -0.4\\,x_j$. 따라서 $w_0 = 0 + (-0.4)(1) = -0.4$, $w_1 = 1 + (-0.4)(2) = 0.2$, $w_2 = 0.5 + (-0.4)(-2) = 1.3$ → $[-0.4,\\,0.2,\\,1.3]$. (참고: 강의 슬라이드의 워크드 예시는 $\\Delta w=\\eta\\,y\\,x$ 관례로 계산해 $[-0.2, 0.6, 0.9]$를 제시했는데, 이는 명시된 델타 규칙보다 학습률이 사실상 2배 작다. 본 문항은 명시된 델타 규칙을 따른다.)",
      "source": "NN PDF; 13주차 § 2.4"
    },
    {
      "id": "MLF13Q16",
  ...
  [
    ...
    },
    {
      ...
      "choices": [
        ...
        "② 독립은 선·비선형 모두 무관한 강조건이고, 무상관은 선형 관계만 없앤다",
