...
export const set9 = [
  ...
  },
  {
    ...
    type: "multiple_choice",
    difficulty: "medium",
    question: "정점이 5개인 완전 그래프가 가질 수 있는 서로 다른 신장트리의 개수는? (완전 그래프 신장트리 개수 = n^(n−2))",
    choices: [
      "16개",
      "25개",
      "125개",
    ...
    answer: 2,
    brief: "n^(n−2) = 5^3 = 125개.",
    detailed: "정점이 n개인 완전 그래프의 서로 다른 신장트리 개수는 케일리 공식 n^(n−2)이다. n=5이면 5^(5−2)=5^3=125개다. 참고로 n=4일 때는 4^2=16개가 된다(첫 번째 보기는 n=4의 값이라 오답).",
  ...
  // ===== 14차시: 검색 (7문항) =====
  {
    ...
    type: "multiple_choice",
    difficulty: "medium",
    question: "정렬된 N개 데이터에서 이진 검색을 할 때 비교 대상이 N → N/2 → N/4 → … 로 반씩 줄어든다. N=2^k라 할 때 데이터가 1개가 될 때까지의 반복 횟수와 그로부터 나오는 시간 복잡도는?",
    choices: [
      "반복 N번, O(N)",
      "반복 N/2번, O(N)",
      "반복 k=log₂N번, O(log N)",
    ...
    answer: 2,
    brief: "2^k→2^0까지 k번 반복, k=log₂N → O(log N).",
    detailed: "비교 대상이 2^k → 2^(k−1) → … → 2^0으로 매번 절반이 되며, 1개가 될 때까지 k번 반복한다. N=2^k의 양변에 밑이 2인 로그를 취하면 k=log₂N이므로 반복 횟수가 log₂N이고 시간 복잡도는 O(log N)이다. 이진 검색은 반드시 정렬된 데이터에서만 쓸 수 있다.",
    source: "14차시 §2 이진 검색"
  },
  {
    id: "DS_S9Q25",
